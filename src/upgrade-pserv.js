...
export const main = async (ns) => {
  ...
  for (const server of servers) {
    ...
    while (ns.getServerMoneyAvailable('home') < ns.getPurchasedServerUpgradeCost(server, memSize)) {
    ...
    const threads = Math.floor(ns.getServerMaxRam(server) / ns.getScriptRam(script, server));
